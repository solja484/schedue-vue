<template>
    <div>
        ...
        <b-alert v-model="showSuccessAlert" variant="success" dismissible class="mt-3 ml-5" fade>
            Розклад {{alertTitle}} успішно відкрито для перегляду студентами!
        </b-alert>
        <b-alert v-model="hideSuccessAlert" variant="info" dismissible class="mt-3 ml-5" fade>
        ...
        <div v-if="type!=subfacultyType">
            <div v-for="l in levels" :key="l.course" class="ml-5">
                ...
                <b-collapse :id="'col-sub-' + type + l.course" class="mt-2">
                    ...
                    <div v-for="sch in schedulesByLevel(l.level, l.year)" :key="sch.id" class="">
                        ...
                        <BSkeleton type="input" class="mx-5 my-2"
                                   v-if="loadingElem && loadingElemId==sch.id"></BSkeleton>
        ...
        <div v-else class="ml-5">
            <div v-for="sch in schedules" :key="sch.id" class="">
                <b-icon-eye-slash v-if="sch.draft==1" class="text-muted text-28 m-2 float-left link"
                                  title="Розклад НЕ доступний для студентів. Натисніть, щоб відкрити доступ."
                                  @click="showSchedule(sch)"></b-icon-eye-slash>
                <b-icon-eye v-else class="text-blue text-28  m-2 float-left"
                            title="Розклад доступний для студентів. Натисніть, щоб закрити доступ."
                            @click="hideSchedule(sch)"></b-icon-eye>
                <BSkeleton type="input" class="mx-5 my-2"
                           v-if="loadingElem && loadingElemId==sch.id"></BSkeleton>
                <ScheduleListELem class="mx-5" v-else
                                  :code="sch.code" :title="sch.title"></ScheduleListELem>
    ...
    export default {
        ...
        computed: {
            ...
            schedulesByLevel: function (level, year) {
                return this.schedules.filter(s => s.level == level && s.study_year == year);
            },
            showSchedule: function (schedule) {
                this.loadingElemId = schedule.id;
                this.$store.dispatch('list/changeDraftMode', {id: schedule.id, draft: 0})
                    .then(() => {
            ...
            hideSchedule: function (schedule) {
                this.loadingElemId = schedule.id;
                this.$store.dispatch('list/changeDraftMode', {id: schedule.id, draft: 1})
                    .then(() => {
                        this.loadingElemId = 0;
                        this.alertTitle = '"'+schedule.title+'"';
                    })
