    ...
    export default {
        ...
        computed: {
            ...
            courses: function () {
                if (this.currentState == CurrentState.SCHEDULE_VIEW)
                    ...
                    return this.$store.getters['edit/editTable'].filter(c => c.day_id == this.day.id);
            },
            /*rows: {
                get: function () {
                    return this.courses;
                ...
                set: function (newCourse) {
                    if (this.currentState == CurrentState.SCHEDULE_VIEW)
                        this.courses = this.$store.getters["schedule/viewRows"].filter(
                            c => c.day_id == this.day.id
                        );
                    else
                        this.courses = this.$store.getters['edit/editTable'].filter(
                            c => c.day_id == this.day.id
                        );
                    if (newCourse) this.courses.push(newCourse);
                }
            },*/
        ...
        mounted() {
          /*  if (this.currentState == CurrentState.SCHEDULE_VIEW)
                ...
                this.courses = this.$store.getters["schedule/editRows"].filter(
                    c => c.day_id == this.day.id
                );*/
        }
    };
