const downloadModule = {
  ...
  state: {
    ...
    sessionHeaders: [
      ...
      { label: "Час", field: "time" },
      { label: "Дисципліна, викладач", field: "course" },
      { label: "Вид контролю", field: "exam" },
      { label: "Група", field: "group" },
      { label: "Дата", field: "weeks" },
  ...
  actions: {
    setDownloadInfo({ commit, dispatch }, info) {
      ...
      dispatch("setRows", {
        ...
        type: info.schedule.schedule_type
      });
    },
    ...
    setRows({ commit, rootGetters }, data) {
      ...
      for (let day of days)
        for (let pair of pairs) {
          let courses = data.courses.filter(
            c => c.pair_id == pair.id && c.day_id == day.id
          );
          let elem = {};
          ...
          if (courses.length > 0) {
            elem["course"] =
              courses[0].teacher == null
                ? courses[0].name
                : courses[0].name + ", " + courses[0].teacher;
            elem["group"] =
              courses[0].group == "0" ? "Лекція" : courses[0].group;
  ...
  mutations: {
    setHeaders(state, type) {
      if (type == "session") state.headers = state.sessionHeaders;
